...
def get_txt_in_dir_by_keywords(keywords: str, dir_path: str) -> str:
    ...
    for filename in os.listdir(dir_path):
        ...
        file_result = get_txt_in_file_by_keywords(keywords, file_full_path)
        result += f"\n\n[{file_full_path}]\n{file_result}"
    logger.info(f"get_txt_in_dir_by_keywords_return, {result}")
    return result


def get_txt_in_file_by_keywords(keywords: str, file_name: str) -> str:
    with open(file_name, 'r', encoding='utf-8') as f:
        ...
        for raw_line in lines:
            ...
            if keywords == clean_line:
                ...
                result.append(full_text[pos: pos + len(raw_line) + 1000])
        return '\n'.join(result)
