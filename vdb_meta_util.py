...
class VdbMeta:
    ...
    def get_vdb_info_by_uid(uid: int, kdb_name='', include_others_public=True):
        ...
        for item in merged_dt:
            if str(item['uid']) == uid:  # 自己的知识库
                ...
                user_name = uid
                try:
                    user_name = DbUtl.get_user_name_by_uid(uid)
                    if not user_name:
                        user_name = uid
    ...
    @staticmethod
    def get_vdb_file_info_by_file_name(file_name: str, uid: str, vdb_id: str) -> list:
    ...
    @staticmethod
    def get_vdb_file_info_by_md5(file_md5: str, uid: str, vdb_id: str) -> list:
